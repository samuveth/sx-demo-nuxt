...
export const useSpacesStore = defineStore('spaces', () => {
  const starredSpacesIds = useStorage(
    `${pkg.name}.spaces-starred`,
    [] as string[]
  )
  ...
  const starredSpacesMap = computed(
    () =>
      new Map(
        starredSpacesData.value.map(space => [
  ...
  const starredSpaces = computed({
    ...
    set(spaces: Space[]) {
      starredSpacesIds.value = spaces.map(
  ...
  function toggleSpaceStar(id: string) {
    if (starredSpacesIds.value.includes(id)) {
      starredSpacesIds.value = starredSpacesIds.value.filter(
