...
export function useActions() {
  ...
  async function wrapPromise(networkId: NetworkID, promise: Promise<any>) {
    ...
    if (envelope.signatureData || envelope.sig) {
      ...
      uiStore.addPendingTransaction(
        receipt.transaction_hash || receipt.hash,
        networkId
      )
    } else {
      uiStore.addPendingTransaction(
  ...
  async function updateMetadata(space: Space, metadata: SpaceMetadata) {
    ...
    const receipt = await network.actions.setMetadata(
      auth.web3,
      space,
      metadata
    )
  ...
  async function receiveProposal(proposal: Proposal) {
    ...
    const network = getNetwork(proposal.network)
    if (!network.hasReceive)
      throw new Error('Receive on this network is not supported')
  ...
  async function executeTransactions(proposal: Proposal) {
    ...
    const receipt = await network.actions.executeTransactions(
      auth.web3,
      proposal
    )
    console.log('Receipt', receipt)
    ...
    const targetNetwork =
      proposal.network === 'sn-tn2' ? 'gor' : proposal.network
    uiStore.addPendingTransaction(
      receipt.transaction_hash || receipt.hash,
      targetNetwork
    )
  ...
  async function executeQueuedProposal(proposal: Proposal) {
    ...
    const receipt = await network.actions.executeQueuedProposal(
      auth.web3,
      proposal
    )
    console.log('Receipt', receipt)
  ...
  async function transferOwnership(space: Space, owner: string) {
    ...
    const receipt = await network.actions.transferOwnership(
      auth.web3,
      space,
      owner
    )
